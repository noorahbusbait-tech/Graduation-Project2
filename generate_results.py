...
# =========================================================
# FIX 1: CLEAN DATE FORMAT (IMPORTANT FOR JSON + CHARTS)
# =========================================================
...
# FIX: ensure clean string dates (prevents 1764633600000 issue)
daily_los['Adm_Date'] = pd.to_datetime(daily_los['Adm_Date']).dt.strftime('%Y-%m-%d')

# -----------------------------
# LOS CHART (IMPROVED + LABELS)
# -----------------------------
plt.figure(figsize=(14, 7))
ax = sns.lineplot(x='Adm_Date', y='LOS', data=daily_los, marker='o')

# add values on top of line
for x, y in zip(daily_los['Adm_Date'], daily_los['LOS']):
    plt.text(x, y, f"{y:.1f}", fontsize=7, rotation=45)

plt.title('Daily Average LOS', fontsize=14)
...
future_dates = pd.date_range(
    start=pd.to_datetime(daily_los['Adm_Date']).max() + pd.Timedelta(days=1),
...
future_forecast_df = pd.DataFrame({
    'Adm_Date': future_dates.strftime('%Y-%m-%d'),
...
# -----------------------------
# FINAL CHART (IMPROVED + LABELS)
# -----------------------------
plt.figure(figsize=(12,6))
ax = sns.lineplot(x='Date', y='Tuned_Predicted_Occupancy', data=final_tuned_df, marker='o')
...
# -----------------------------
# DEMAND CHART (IMPROVED + LABELS)
# -----------------------------
plt.figure(figsize=(12,6))
ax = sns.lineplot(x='Adm_Date', y='Occupancy', data=daily_occ, marker='o')

for x, y in zip(daily_occ['Adm_Date'].astype(str), daily_occ['Occupancy']):
    plt.text(x, y, f"{y:.1f}", fontsize=6, rotation=45)
